...
func NewSuiteFS(tb testing.TB, vfsSetup avfs.VFS, opts ...Option) *SuiteFS {
	...
	// Make sur rootDir does not include symbolic links.
...
func (sfs *SuiteFS) RunTests(t *testing.T, userName string, testFuncs ...TestFunc) {
	...
	for _, tf := range testFuncs {
		testDir := sfs.CreateTestDir(t, tf)
...
// CreateTestDir creates and returns the base directory for the tests.
func (sfs *SuiteFS) CreateTestDir(tb testing.TB, testFunc TestFunc) string {
	vfs := sfs.vfsSetup
...
func (sfs *SuiteFS) ClosedFile(tb testing.TB, testDir string) (f avfs.File, fileName string) {
	...
	vfs := sfs.vfsSetup
	fileName = vfs.Join(testDir, "closedFile")
...
// ExistingFile returns an existing file with the given content.
