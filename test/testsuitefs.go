...
type SuiteFS struct {
	...
	benchRoot   string             // benchRoot is the root directory for benchmarks.
...
func (sfs *SuiteFS) createTestRoot(tb testing.TB) {
	...
	if err != nil {
		tb.Fatalf("MkdirTemp %s : want error to be nil, got %s", testRoot, err)
	}

	// Default permissions on testRoot are 0o700 (generated by MkdirTemp()).
	// Make testRoot accessible by anyone.
	err = vfs.Chmod(testRoot, 0o777)
	if err != nil {
		tb.Fatalf("Chmod %s : want error to be nil, got %s", testRoot, err)
	}

	// Ensure testRoot does not include symbolic links.
	if vfs.HasFeature(avfs.FeatSymlink) {
		testRoot, err = vfs.EvalSymlinks(testRoot)
		if err != nil {
			tb.Fatalf("EvalSymlinks %s : want error to be nil, got %s", testRoot, err)
		}
	}

	sfs.testRoot = testRoot
}

// createBenchRoot creates benchmarks root directory.
func (sfs *SuiteFS) createBenchRoot(tb testing.TB) {
	vfs := sfs.vfsSetup

	// run Benches on real disks, /tmp is usually an in memory file system.
	benchRoot := vfs.Join(vfs.HomeDirUser(vfs.User()), "tmp")

	err := vfs.Mkdir(benchRoot, avfs.DefaultDirPerm)
	if err != nil {
		tb.Fatalf("Mkdir %s : want error to be nil, got %s", benchRoot, err)
	}
...
func (sfs *SuiteFS) RunTests(t *testing.T, userName string, testFuncs ...SuiteTestFunc) {
	...
	for _, tf := range testFuncs {
		...
		fn := funcName(tf)
		testDir := vfs.Join(sfs.testRoot, fn)
	...
	sfs.RemoveTestDir(t, sfs.testRoot)
}
...
func (sfs *SuiteFS) RunBenches(b *testing.B, userName string, benchFuncs ...SuiteBenchFunc) {
	...
	for _, bf := range benchFuncs {
		...
		fn := funcName(bf)
		benchDir := vfs.Join(sfs.benchRoot, fn)

		sfs.CreateDir(b, benchDir, 0o777)
		sfs.ChangeDir(b, benchDir)

		b.Run(fn, func(b *testing.B) {
			bf(b, benchDir)
		})

		sfs.RemoveTestDir(b, benchDir)
	}

	sfs.RemoveTestDir(b, sfs.benchRoot)
}
...
// BenchStart starts a benchmark.
func (sfs *SuiteFS) BenchStart(b *testing.B) {
	sfs.cleanCache(b)
	b.ReportAllocs()
	b.ResetTimer()
	b.StartTimer()
...
func (sfs *SuiteFS) BenchStop(b *testing.B) {
	b.StopTimer()

	vfs := sfs.vfsSetup

	err := vfs.RemoveAll(sfs.benchRoot)
	if err != nil {
		b.Fatalf("Can't remove %s : %v", sfs.benchRoot, err)
	}
}

// cleanCache cleans disk cache for benches (available on linux only).
func (sfs *SuiteFS) cleanCache(tb testing.TB) {
	vfs := sfs.vfsSetup

	if !vfs.HasFeature(avfs.FeatRealFS) || vfs.OSType() != avfs.OsLinux || !sfs.initUser.IsAdmin() {
		return
	}

	err := os.WriteFile("/proc/sys/vm/drop_caches", []byte("3"), avfs.DefaultFilePerm)
	CheckNoError(tb, "Clean cache", err)
}
...
func (sfs *SuiteFS) RemoveTestDir(tb testing.TB, testDir string) {
	...
	if err != nil {
		tb.Fatalf("Chdir %s : want error to be nil, got %v", sfs.testRoot, err)
	}
